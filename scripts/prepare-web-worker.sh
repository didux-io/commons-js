...
sed -n "$LamportGeneratorStart,$LamportGeneratorEnd p" ./build/merkle/LamportGenerator.js > ./build/merkle/LamportGenerator.stripped.js
sed -n "$SHA1PRNGStart,$SHA1PRNGEnd p" ./build/random/SHA1PRNG.js > ./build/random/SHA1PRNG.stripped.js

# Fix SHA1PRNG being renamed
sed -i '' -e 's/SHA1PRNG_1.SHA1PRNG/SHA1PRNG/g' ./build/merkle/LamportGenerator.stripped.js
