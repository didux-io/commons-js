...
describe("BIP39", () => {
    ...
    beforeEach(() => {
        ...
        activeWordListSpy = spyOn(<any>bip39, "initialize").and.callFake(
            () => {
                (<any>bip39).activeWordList = wordList.split("\n")
            }
        );
    })

    it("should generate correct mnemonic phrases", () => {
        let testIndex = 0;

        // Spy on the (private) getRandomBytes method. Instead of
        // returning true random bytes we return a predefined value.
        spyOn(<any>bip39, "getRandomBytes").and.callFake(() => {
        ...
        for(let i = 0; i < phraseTestVectors.english.length; i++) {
            ...
            generatedPhrases.push(bip39.generate(strength));
        }
    ...
    it("should generate correct seeds", () => {
        for(let i = 0; i < phraseTestVectors.english.length; i++) {
            ...
            let seed = bip39.toSeed(testVector[1], passphrasePassword);
    ...
    it("should throw an exception because the entropy size in bits has to be divisible by 32", () => {
        try {
            bip39.generate(15);
            expect(true).toBeFalsy("Exception should be thrown");
        }
        catch(ex) {
    ...
    it("should return 4 spaces of u3000 when 5 words in japanese", () => {
        (<any>bip39).language = "japanese";
        let result = (<any>bip39).joinWords(["word1", "word2", "word3", "word4", "word5"]);
